...
SAT.Edge.prototype.ToLine = function () {
  return new SAT.Line(
    this.__shape.Vertex(this.__startIndex),
    this.__shape.Vertex(this.__endIndex)
  );
};
...
SAT.Shape.prototype.Consolidate = function (shape) {
  ...
  for (var i = 0; i < this.__faces.length; i++) {
    faces.push(new SAT.Face(shape, this.__faces[i]));
  }
  var edges = [];
  for (var i = 0; i < this.__edges.length; i++) {
    edges.push(
      new SAT.Edge(
        shape,
        this.__edges[i][0],
        this.__edges[i][this.__edges[i].length - 1]
      )
    );
  }
...
SAT.Shape.prototype.Rotate = function (x, y, z) {
  this.__matrix = new THREE.Matrix4().multiplyMatrices(
    this.__matrix,
...
SAT.Shape.prototype.SetRotation = function (x, y, z) {
  ...
  transform
    ...
    .setPosition(new THREE.Vector3(this.GetX(), this.GetY(), this.GetZ()));
...
SAT.Line.prototype.__Constructor = function (start, end) {
  ...
  SAT.Line.parent.__Constructor.call(
    this,
    [start, end],
    [],
    [new SAT.Edge(this, 0, 1)]
  );
};
...
SAT.Polygon.prototype.__Constructor = function (vertices) {
  ...
  SAT.Polygon.parent.__Constructor.call(
    this,
    vertices,
    [new SAT.Face(this, System.Array.Range(0, vertices.length))],
    edges
  );
  this.__normal = null;
...
SAT.Polygon.prototype.ComputeNormal = function () {
  ...
  for (
    var i = this.__vertices.length - 1, j = 0;
    j < this.__vertices.length;
    i = j, j++
  ) {
    this.__normal.x +=
      (this.__vertices[i].z + this.__vertices[j].z) *
      (this.__vertices[i].y - this.__vertices[j].y);
    this.__normal.y +=
      (this.__vertices[i].x + this.__vertices[j].x) *
      (this.__vertices[i].z - this.__vertices[j].z);
    this.__normal.z +=
      (this.__vertices[i].y + this.__vertices[j].y) *
      (this.__vertices[i].x - this.__vertices[j].x);
...
SAT.Polygon.prototype.ComputeCentroid = function () {
  var e1 = new THREE.Vector3()
    .subVectors(this.__vertices[1], this.__vertices[0])
    .normalize();
  ...
  area *= 0.5;
  this.__centroid = SAT.Polygon.ToGlobalCoordinates(
    e1,
    e2,
    origin,
    centroid.divideScalar(6.0 * area)
  );
};
...
SAT.Polygon.ToLocalCoordinates = function (e1, e2, origin, vertex) {
  var x =
    (vertex.x - origin.x) * e1.x +
    (vertex.y - origin.y) * e1.y +
    (vertex.z - origin.z) * e1.z;
  var y =
    (vertex.x - origin.x) * e2.x +
    (vertex.y - origin.y) * e2.y +
    (vertex.z - origin.z) * e2.z;
  return new THREE.Vector2(x, y);
};

SAT.Polygon.ToGlobalCoordinates = function (e1, e2, origin, vertex) {
  return new THREE.Vector3(
    origin.x + e1.x * vertex.x + e2.x * vertex.y,
...
SAT.Box.prototype.__Constructor = function (width, height, depth) {
  if (
    System.Type.IsUndefined(width) ||
    System.Type.IsUndefined(height) ||
    System.Type.IsUndefined(depth)
  ) {
  ...
  var edges = [
    new SAT.Edge(this, 0, 1),
    new SAT.Edge(this, 1, 2),
    new SAT.Edge(this, 2, 3),
    new SAT.Edge(this, 3, 0),
    new SAT.Edge(this, 4, 5),
    new SAT.Edge(this, 5, 6),
    new SAT.Edge(this, 6, 7),
    new SAT.Edge(this, 7, 4),
    new SAT.Edge(this, 3, 4),
    new SAT.Edge(this, 2, 5),
    new SAT.Edge(this, 0, 7),
    new SAT.Edge(this, 1, 6),
  ];
...
SAT.Box.prototype.Clone = function () {
  ...
  for (var i = 0; i < this.__edges.length; i++) {
    edges.push(
      new SAT.Edge(
        clone,
        this.__edges[i].StartIndex(),
        this.__edges[i].EndIndex()
      )
    );
  }
...
SAT.BuildMesh = function (shape0, params) {
  ...
  if (
    !System.Type.IsUndefined(params) &&
    !System.Type.IsUndefined(params.decoration) &&
    params.decoration
  ) {
    for (var i = 0; i < shape1.VertexCount(); i++) {
      var vertexMesh = new THREE.Mesh(
        new THREE.SphereGeometry(0.06, 12, 6),
    ...
    for (var i = 0; i < shape1.EdgeCount(); i++) {
      ...
      edgesGeometry.merge(
        new THREE.CylinderGeometry(0.02, 0.02, edge.Length(), 8, 4),
        new THREE.Matrix4()
          .makeRotationFromQuaternion(arrowHelper.quaternion)
          .setPosition(
            new THREE.Vector3().addVectors(
              edge.Start(),
              direction.multiplyScalar(edge.Length() * 0.5)
            )
          ),
        0
      );
  ...
  var interiorFacesMesh = new THREE.Mesh(
    facesGeometry,
    new THREE.MeshBasicMaterial({
      color: 0xffffff,
      vertexColors: THREE.FaceColors,
  ...
  for (var i = 0; i < a1.EdgeCount(); i++) {
    ...
    for (var j = 0; j < b1.EdgeCount(); j++) {
      ...
      var D = new THREE.Vector3()
        .crossVectors(e0.GetDirection(), e1.GetDirection())
        .normalize();
      var same0;
